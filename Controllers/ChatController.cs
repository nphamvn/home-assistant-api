...
public class ChatController : ControllerBase
{
    ...
    public async Task<IActionResult> GetConversation([FromQuery] int? id, [FromQuery] string? participant)
    {
        ...
        {
            var conversation = await _conversationRepository.Single(c => c.Creator.UserName == participant && c.Partner.UserName == username ||
                                                                      c.Creator.UserName == username && c.Partner.UserName == participant);
            return Ok(conversation);
        }
        else
        {
